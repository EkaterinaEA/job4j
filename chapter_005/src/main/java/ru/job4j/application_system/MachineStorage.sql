...
insert into engines (engine, model)
values ('petrol', 'A4'),
       ('diesel', 'BMW 5'),
       ('electric', 'Tesla X');
insert into car_bodies (car_body, model)
values ('coupe', 'A4'),
       ('hardtop', 'BMW 5'),
       ('hatchback', 'Tesla X');
insert into transmissions (transmission, model)
values ('auto', 'A4'),
       ('manual', 'BMW 5'),
       ('variator', 'Tesla X');
insert into cars (brand, model, engine_id, car_body_id, transmissions_id)
values ('Audi', 'A4', 1, 1, 1),
       ('BMW', '5', 2, 2, 2);
...
Select cb.id, cb.car_body, cb.model
from car_bodies as cb
Left Outer Join cars as c on c.car_body_id = cb.id
where car_body_id is null;

Select t.id, t.transmission, t.model
from transmissions as t
Left Outer Join cars as c on c.transmissions_id = t.id
